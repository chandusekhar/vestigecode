...
namespace WSS.Common.Utilities.ActionLink
{
    ...
    public class ActionLinkManager : IActionLinkManager
    {
        ...
        public ActionLinkManager(IUnitOfWork unitOfWork, string baseUrl)
        {
        ...
        public string GenerateResetPasswordLink(int wssAccountId)
        {
            ...
            _unitOfWork.Save("0");
            return $"{_baseUrl + "Password/ResetPassword?activationToken="}{action.ActionToken}&accountId=" + wssAccountId;
        }
        ...
        public string GenerateActivateAccountLink(int wssAccountId)
        {
            ...
            _unitOfWork.ActionDataRepository.Insert(action);
            _unitOfWork.Save("0");
            return $"{_baseUrl}AccountActivation/?key={action.ActionToken}";
        }
        ...
        public string GenerateUnsubscribeSecondaryLink(int wssAccountId, string secondaryEmailAddress)
        {
            ...
            return additionalEmailAddress == null
                ? null
                : $"{_baseUrl}{wssAccount.AuthenticationId.ToString().Replace("-", string.Empty)}?A=US;SA={HttpUtility.UrlEncode(secondaryEmailAddress)}";
        }
        ...
        public string GenerateUnsubscribePrimaryLink(int wssAccountId)
        {
            ...
            return wssAccount == null
                ? null
                : $"{_baseUrl}{wssAccount.AuthenticationId.Replace("-", string.Empty)}?A=UP";
        }
    }
}